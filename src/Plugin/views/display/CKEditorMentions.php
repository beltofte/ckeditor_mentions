...
/**
 * @file
 * Contains \Drupal\ckeditor_mentions\Plugin\views\display\CKEditorMentions.
 */

namespace Drupal\ckeditor_mentions\Plugin\views\display;
...
/**
 * The plugin that handles a CKEditor Mentions display.
 *
 * "ckeditor_mentions_display" is a custom property, used with
 * \Drupal\views\Views::getApplicableViews() to retrieve all views with a
 * 'CKEditor Mentions' display.
 *
 * @ingroup views_display_plugins
 *
 * @ViewsDisplay(
 *   id = "ckeditor_mentions",
 *   title = @Translation("CKEditor Mentions"),
 *   admin = @Translation("CKEditor Mentions Source"),
 *   help = @Translation("Selects mentionable users for the CKEditor Mentions plugin."),
 *   theme = "views_view",
 *   register_theme = FALSE,
 *   uses_menu_links = FALSE,
 *   base = {"users_field_data"},
 *   ckeditor_mentions_display = TRUE
 * )
 */
class CKEditorMentions extends DisplayPluginBase {
  ...
  protected function defineOptions() {
    ...
    // Force the style plugin to 'ckeditor_mentions_style' and the row plugin to
    // 'fields'.
    $options['style']['contains']['type'] = array('default' => 'ckeditor_mentions');
    $options['defaults']['default']['style'] = FALSE;
    $options['row']['contains']['type'] = array('default' => 'ckeditor_mentions');
  ...
  public function validate() {
    ...
    if (!isset($style['options']['search_fields'])) {
      $errors[] = $this->t('Display "@display" needs a selected search fields to work properly. See the settings for the CKEditor Mentions list format.', array('@display' => $this->display['display_title']));
    }
    else {
      ...
      foreach ($style['options']['search_fields'] as $field_alias => $enabled) {
        if ($enabled && !in_array($field_alias, $fields)) {
          $errors[] = $this->t('Display "@display" uses field %field as search field, but the field is no longer present. See the settings for the CKEditor Mentions list format.', array('@display' => $this->display['display_title'], '%field' => $field_alias));
        }
      }
    }
    return $errors;
